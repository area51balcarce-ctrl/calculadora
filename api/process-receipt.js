...
function norm(t){return String(t||'').replace(/\r/g,'\n').replace(/[ \t]+/g,' ').replace(/\n{2,}/g,'\n').trim()}
function parseMoney(v){
  if(!v)return 0;
  let s=String(v).replace(/\$/g,'').replace(/\s/g,'').replace(/[^\d,.-]/g,'');
  const c=s.includes(','), d=s.includes('.');
  if(c&&d)s=s.replace(/\./g,'').replace(',','.');
  else if(c)s=s.replace(',','.');
  else if(d){const p=s.split('.'); if(p.length>2){const last=p.pop(); s=p.join('')+'.'+last;}}
  const n=Number(s); return Number.isFinite(n)?n:0;
}
const MONEY=/-?\$?\s*\d{1,3}(?:\.\d{3})*,\d{2}|-?\$?\s*\d+(?:,\d{2})|-?\$?\s*\d{1,3}(?:\.\d{3})+\.\d{2}|-?\$?\s*\d+\.\d{2}/g;
function nums(line){return (String(line).match(MONEY)||[]).map(parseMoney).filter(Number.isFinite)}
function lastMoney(line){const n=nums(line);return n.length?n[n.length-1]:0}
function clean(l){return String(l||'').replace(/\s+/g,' ').trim()}
function findIdx(lines,rx){return lines.findIndex(l=>rx.test(l))}
function findAmount(lines,rx){for(const l of lines){if(rx.test(l)){const v=lastMoney(l); if(v>0)return v}}return 0}
  ...
  for(const line of lines){
    ...
    if(n.length>=3){
      ...
      if(habCAp>100000 && totalDesc>10000){
        best={haberesConAporte:round2(habCAp),haberesSinAporte:round2(habSAp),totalDescuentos:round2(totalDesc),raw:line};
      }
    }
  }
  return best;
...
function findHaberes(lines){
  ...
  if(ipsIdx>0){
    ...
    for(let i=0;i<ipsIdx;i++){
      ...
      if(/sueldo|basico|básico|jornada|antig[uü]edad|refrigerio|horario|presentismo|horas|bonif|adicional|guardia/i.test(l)){
        const v=lastMoney(l); if(v>0)sum+=v;
      }
    }
  ...
  for(const l of lines){
    if(/hab\.?\s*c\/ap|haberes?\s*con\s*aporte/i.test(l)){
      const v=lastMoney(l); if(v>0)return v;
    }
  }
  return 0;
}
...
function extractIoma(lines){
  ...
  const v=lastMoney(lines[idx]); if(v>0)return{index:idx,amount:v};
  for(let i=idx+1;i<=Math.min(idx+2,lines.length-1);i++){const x=lastMoney(lines[i]); if(x>0)return{index:idx,amount:x}}
  return{index:idx,amount:0};
}

function isStop(l){return /(son\s+pesos|liquido\s+a\s+pagar|líquido\s+a\s+pagar|neto\s+a\s+cobrar|neto|liquido|líquido|firma|recibi|recibí|banco|cuenta|cbu)/i.test(l)}
function conceptItem(line){
  const n=nums(line); if(!n.length)return null;
...
function ignoreConcept(c){return /(hab\.?\s*c\/ap|haberes?|i\.?\s*p\.?\s*s|ips|i\.?\s*o\.?\s*m\.?\s*a|ioma|neto|liquido|líquido|total|totales|son\s+pesos)/i.test(c)}
function discountsByLines(lines,iomaIdx){
  if(iomaIdx<0)return[];
  const out=[];
  for(let i=iomaIdx+1;i<lines.length;i++){
    const l=clean(lines[i]); if(!l)continue; if(isStop(l))break;
    const item=conceptItem(l); if(!item)continue; if(ignoreConcept(item.concept))continue;
    out.push(item);
  }
  return out;
}
...
function calculate(raw){
  ...
  return{
    ...
    debug:{
      ...
      totals_row:totals,
      resumen:{haberes,ips,ioma,resultado_x:resultadoX,base_75:base75,total_descuentos:totalDebajoIoma,total_descuentos_recibo:totals?totals.totalDescuentos:0},
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!String(parsed.text||'').trim())return res.status(422).json({success:false,message:'El PDF no tiene texto seleccionable. Usá el PDF original, no una foto o escaneo.'});
    return res.status(200).json(calculate(parsed.text));
  }catch(e){
    return res.status(500).json({success:false,message:'Error al procesar el PDF.',error:e.message});
  }
}
